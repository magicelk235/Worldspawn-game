import sys,random,pygame,os,items,entities,objects,math,gif_pygame,projectiles,pickle,struct,copy,gif_pygame,zlib,numpy,socket,particles,events,enum,game_events
import uuid
...
class AliveObject(GameObject):
    ...
    def applyDamage(self, game, damage, attacker=None):
        if self.timers.get("damage", None) == None:
            ...
            self.damageEffectOn()
        if attacker != None and not has_one_tag(self, attacker):
